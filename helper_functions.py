...
def plot_random_images(target_dir, scale = 3, rows=3, cols=6):
  """
  Plots random images and return the address of a single random image.
  Args:
      target_dir: path of the training or validation directory
      scale: scale at which images are to be plotted. Recommeneded value is > 1
      rows: number of rows in the image grid
      cols: number of columns in the image grid
  Returns:
      random_img_path: path of the last image in the grid
  """
  ...
  for i in range(rows*cols):
    random_img_path = random.sample(images,1)[0]
    img = mpimg.imread(random_img_path)
    ax = plt.subplot(rows,cols,i+1)
    plt.imshow(img, cmap=plt.cm.binary)
    plt.title(random_img_path.split(os.sep)[-2])
